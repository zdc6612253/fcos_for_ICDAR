...
class BoxList(object):
    ...
    def resize(self, size, *args, **kwargs):
        ...
        bbox = BoxList(scaled_box, size, mode="xyn")
        # bbox._copy_extra_fields(self)
        for k, v in self.extra_fields.items():
            if not isinstance(v, torch.Tensor):
                v = v.resize(size, *args, **kwargs)
    ...
    def transpose(self, method):
        ...
        bbox = BoxList(transposed_boxes, self.size, mode="xyn")
        # bbox._copy_extra_fields(self)
        for k, v in self.extra_fields.items():
            if not isinstance(v, torch.Tensor):
    ...
    def clip_to_image(self, remove_empty=True):
        ...
        self.bbox[:, 5].clamp_(min=0, max=self.size[0] - TO_REMOVE)
        self.bbox[:, 6].clamp_(min=0, max=self.size[0] - TO_REMOVE)
        self.bbox[:, 7].clamp_(min=0, max=self.size[0] - TO_REMOVE)
        if remove_empty:
            box = self.bbox
