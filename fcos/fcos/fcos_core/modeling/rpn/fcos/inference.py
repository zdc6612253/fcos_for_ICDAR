...
class FCOSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(
        ...
        pre_nms_top_n = candidate_inds.view(N, -1).sum(1)  # 8,1 代表每个N的大于score的数量
        ...
        results = []
        for i in range(N):  # 不同的图片
            per_box_cls = box_cls[i]  # w*h,c 66,1
            per_candidate_inds = candidate_inds[i]  # w*h*c  66,1 True,False
            per_box_cls = per_box_cls[per_candidate_inds]  # shape:66 代表正样本的分数

            per_candidate_nonzeros = per_candidate_inds.nonzero()  # True的位置  num,2  [index,0],,,
            # print(per_candidate_nonzeros.shape)
            # print(per_candidate_nonzeros)
            per_box_loc = per_candidate_nonzeros[:, 0]  # 索引 Num
            per_class = per_candidate_nonzeros[:, 1] + 1  # 类别 Num
    ...
    def forward(self, locations, box_cls, box_regression, centerness, image_sizes):
        ...
        if not self.bbox_aug_enabled:  # not false: 后处理步骤
